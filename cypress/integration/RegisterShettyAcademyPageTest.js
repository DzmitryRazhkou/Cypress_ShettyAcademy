...
describe("Shetty Academy Register Page Test", () => {
  ...
  it("Title Of Page, Headers Validation", () => {
    ...
    registerShettyAcademyPage
      .socialMediaLinks()
      .should("have.length", userDetails.quantity);
    cy.log(
      "The quantity of the social media links are " + userDetails.quantity
    );

    registerShettyAcademyPage
      .getEmailAcademy()
      .should("have.text", userDetails.academyEmail)
      .then((element) => {
  ...
  it("Login With Incorrect Credentials", () => {
    ...
    registerShettyAcademyPage.getIncorrectLoginMessage().then(($div) => {
      const loginSuccessMessageText = $div.text();
      expect(loginSuccessMessageText.includes("Incorrect email or password."))
        .to.be.true;
  ...
  it("Register New User", () => {
    ...
    registerShettyAcademyPage.getSuccessRegistrationMessage().then((ele) => {
      const successRegistrationMessageText = ele.text();
      expect(
        successRegistrationMessageText.includes("Account Created Successfully")
      ).to.be.true;
      cy.log(
        "Login incorrect success message is: " + successRegistrationMessageText
      );
    });
  });
});
